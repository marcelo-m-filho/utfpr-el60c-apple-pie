...
typedef struct AUDIO_Node
{
  ...
  struct AUDIO_Session* session_handle;  // the main session where this node belongs
...
typedef enum 
{
  ...
  AUDIO_OVERRUN_TH_REACHED,   // an overrun threshold is reached: overrun is soon but not yet reproduced on the circular buffer
  AUDIO_UNDERRUN_TH_REACHED,  // an underrun threshold is reached: underrun is soon but not yet reproduced on the circular buffer
