...
/**
  ...
  */
int8_t AUDIO_PlaybackSessionInit(USBD_AUDIO_Streaming_interface_t* usb_device_audio_streaming_p, USBD_AUDIO_Control_t* controls_desc, uint8_t* control_count, uint32_t session_handle)
{
  ...
  *control_count = 0;
  ...
  USB_AudioStreamingInputInit(&usb_device_audio_streaming_p->data_ep,  &PlaybackAudioDescription,  &play_session->session,  (uint32_t)&PlaybackUSBInputNode);
...
#if USE_AUDIO_PLAYBACK_USB_FEEDBACK
     usb_device_audio_streaming_p->synch_enabled = 1;
     usb_device_audio_streaming_p->synch_ep.ep_num = USB_AUDIO_CONFIG_PLAY_EP_SYNC;
     usb_device_audio_streaming_p->synch_ep.GetFeedback = USB_AudioPlaybackGetFeedback;
     usb_device_audio_streaming_p->synch_ep.private_data = (uint32_t) play_session;
     usb_device_audio_streaming_p->SofReceived = AUDIO_USB_Session_Sof_Received;
#endif /* USE_AUDIO_PLAYBACK_USB_FEEDBACK */
  /* set USB AUDIO class callbacks */
  usb_device_audio_streaming_p->interface_num =  play_session->interface_num;
  usb_device_audio_streaming_p->alternate = 0;
  usb_device_audio_streaming_p->max_alternate = AUDIO_USB_PLAYBACK_ALTERNATE;
  usb_device_audio_streaming_p->private_data = session_handle;
  usb_device_audio_streaming_p->SetAS_Alternate = USB_AudioPlaybackSetAudioStreamingInterfaceAlternateSetting;
  usb_device_audio_streaming_p->GetState = USB_AudioPlaybackGetState;
