...
/**
  * @brief  Initializes the audio speaker node, sets callbacks and start the codec. 
  *         Since no data is ready, the SAI is fed from the alternate buffer (filled by zeros)
...
/**
  * @brief  Manages the DMA error event.
  * @param  None
  * @retval None
  */
...
void BSP_AUDIO_OUT_TransferComplete_CallBack(void)
{
  ...
  if((AUDIO_SpeakerHandler) && (AUDIO_SpeakerHandler->node.state != AUDIO_NODE_OFF))
  {
   ...
   // if any stop command was received (?)
   ...
   if(AUDIO_SpeakerHandler->specific.cmd & SPEAKER_CMD_CHANGE_FREQUENCE)
   {
    ...
    if(AUDIO_SpeakerHandler->node.state == AUDIO_NODE_STARTED)
    {
      ...
      // informs session that a packed has been played
      AUDIO_SpeakerHandler->node.session_handle->SessionCallback(AUDIO_PACKET_PLAYED, (AUDIO_Node_t*)AUDIO_SpeakerHandler, AUDIO_SpeakerHandler->node.session_handle);


      // prepares next size to be injected
      #if (USB_AUDIO_CONFIG_PLAY_RES_BIT == 24)
      ...
      {
        // informs session that an underrun has happened
        AUDIO_SpeakerHandler->node.session_handle->SessionCallback(AUDIO_UNDERRUN, (AUDIO_Node_t*)AUDIO_SpeakerHandler, AUDIO_SpeakerHandler->node.session_handle);
      }
      else
      {
        #if (USB_AUDIO_CONFIG_PLAY_RES_BIT == 24)
...
/**
  * @brief  This function is called when half of the requested buffer has been transferred.
  * @param  None
  * @retval None
  */
...
/**
  * @brief  De-Initializes the audio speaker node 
...
/**
  * @brief  Starts the audio speaker node 
...
static int8_t  AUDIO_SpeakerStart(AUDIO_CircularBuffer_t* buffer,  uint32_t node_handle)
{
 ...
 /**
  * @brief  Stop speaker node. The speaker will be stopped after finalizing current packet transfer.
...
static int8_t  AUDIO_SpeakerStop(uint32_t node_handle)
{
 ...
 /**
  * @brief  Changes frequency then stops speaker node
  * @param  node_handle [in]: speaker node handle (must be Started)
  * @retval 0 if no error
  */  
static int8_t  AUDIO_SpeakerChangeFrequency(uint32_t node_handle)
...
static void  AUDIO_SpeakerInitInjectionsParams(AUDIO_SpeakerNode_t* speaker)
{
  ...
  // updates alternative buffer
  memset(speaker->specific.alt_buffer, 0, speaker->specific.injection_size);
  speaker->specific.data      = speaker->specific.alt_buffer; // starts injection of dumped data
  speaker->specific.data_size = speaker->specific.injection_size;
 }

 /**
  * @brief  Sets Mute value to the speaker
...
static int8_t  AUDIO_SpeakerMute( uint16_t channel_number,  uint8_t mute , uint32_t node_handle)
{
 ...
 /**
  * @brief  Sets Volume value to speaker
  * @param  channel_number(IN): channel number
...
#if USB_AUDIO_CONFIG_PLAY_RES_BIT == 24  
  /**
    * @brief  padding 24bit sample to 32 sample by adding zeros.
    * @param  buff_src(IN):          
    * @param  data_dest(OUT):          
    * @param  size(IN):               
    * @retval None
...
static int8_t  AUDIO_SpeakerStartReadCount( uint32_t node_handle)
{
 ...
 /**
  * @brief  returns the number of bytes that have been read and resets the counter
...
static uint16_t  AUDIO_SpeakerGetLastReadCount(uint32_t node_handle)
{
  ...
  int cur_waiting_bytes, read_bytes, last_packet_size;
   ...
   // reads remainding value in dma buffer
