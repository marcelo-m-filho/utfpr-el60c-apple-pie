...
typedef enum USBD_AUDIO_SpecificACInterfaceDescSubtype
{
...
USBD_AUDIO_SpecificACInterfaceDescSubtype_t;

typedef enum USBD_AUDIOTerminal
{
...
USBD_AUDIOTerminal_t;

// Audio Streaming Interface Descriptor Subtypes 
typedef enum USBD_AUDIO_SpecificASInterfaceDescSubtype
{
...
USBD_AUDIO_SpecificASInterfaceDescSubtype_t;

// The feature Unit callbacks 
typedef struct USBD_AUDIO_FeatureControlCallbacks
{
   ...
   int8_t  (*GetCurVolume)  (uint16_t /*channel*/,  uint16_t* /*volume*/, int32_t  /* privatedata*/);
   uint16_t MaxVolume;
   uint16_t MinVolume;
   uint16_t ResVolume;
   int8_t  (*GetStatus)     (uint32_t /*privatedata*/);
}
USBD_AUDIO_FeatureControlCallbacks_t;


// the Unit callbacks , used when a control is called (Get_Cur, Set Cur ....) 
typedef union USBD_AUDIO_ControlCallbacks
{
   USBD_AUDIO_FeatureControlCallbacks_t* feature_control;
}
USBD_AUDIO_ControlCallbacks_t;

//* Audio Unit supported cmd and related callbacks 


// defines an audio Unit 
typedef struct USBD_AUDIO_Control
{
    uint8_t id;                                       // Unit Id 
    USBD_AUDIO_SpecificACInterfaceDescSubtype_t type; // type of Unit 
    ...
    USBD_AUDIO_ControlCallbacks_t Callbacks;          // list of callbacks 
...
USBD_AUDIO_Control_t;
 ...
 // defines a data endpoint and it's callbacks 
typedef struct USBD_AUDIO_EP_Data
{
  uint8_t    ep_num; 
...
typedef struct USBD_AUDIO_Streaming_interface
{
  ...
  USBD_AUDIO_EP_Data_t  data_ep;        // audio streaming interface main data EP  
  #if USBD_SUPPORT_AUDIO_OUT_FEEDBACK 
  ...
  int8_t                (*GetState)         (uint32_t/*privatedata*/);
  uint32_t              private_data;       // used as the last argument of each callback       
}
USBD_AUDIO_Streaming_interface_t;


// Structure define the whole audio function will be initialized by application
typedef struct USBD_AUDIO_FunctionDescriptionf
{
  uint8_t                   control_count; // the count  of Unit controls 
  uint8_t                   as_interfaces_count;// the count  of audio streaming interface 
  USBD_AUDIO_Control_t      controls[USBD_AUDIO_CONFIG_CONTROL_UNIT_COUNT]; // list of Unit control 
  USBD_AUDIO_Streaming_interface_t as_interfaces[USBD_AUDIO_AS_INTERFACE_COUNT];// the list  of audio streaming interface 
}
USBD_AUDIO_FunctionDescriptionf_t;

// Structure define audio interface 
typedef struct USBD_AUDIO_InterfaceCallbacksf
{
    int8_t  (*Init)           (USBD_AUDIO_FunctionDescriptionf_t* /* as_desc*/ , uint32_t /*privatedata*/);
    int8_t  (*DeInit)         (USBD_AUDIO_FunctionDescriptionf_t* /* as_desc*/,uint32_t /*privatedata*/);
    int8_t  (*GetConfigDesc)  (uint8_t ** /*pdata*/, uint16_t * /*psize*/, uint32_t /*private_data*/);
    int8_t  (*GetState)       (uint32_t privatedata);
    uint32_t private_data;  
}
USBD_AUDIO_InterfaceCallbacksf_t;
...
// exported variables --------------------------------------------------------
extern USBD_Class_t USBD_AUDIO;
#define USBD_AUDIO_CLASS &USBD_AUDIO

// exported functions --------------------------------------------------------
uint8_t  USBD_AUDIO_RegisterInterface  (USBD_Handles_t *pdev, USBD_AUDIO_InterfaceCallbacksf_t *aifc);
